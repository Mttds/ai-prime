...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
        ...
        if mother is None and father is None:
            ...
            if genes == 2:
                prob_gene = passing_prob[mother] * passing_prob[father]
            elif genes == 1:
                prob_gene = passing_prob[mother] * (1 - passing_prob[father]) + (1 - passing_prob[mother]) * passing_prob[father]
            else:
                prob_gene = (1 - passing_prob[mother]) * (1 - passing_prob[father])
...
def normalize(probabilities):
    ...
    for person in probabilities:
        total = 0
        for i in range(3): # 0, 1, 2 copies of the gene
